...
export interface RAZHistoryEntry {
  ...
  // 🆕 HTML complet de la feuille de caisse (pour réaffichage identique)
  cashSheetHtml?: string;
...
export class MyConfortDB extends Dexie {
  ...
  vendorCommissionArchives!: Table<any>; // Archives des tableaux de commission
  ...
  constructor() {
    ...
    // ------------------------------------------------------------------------
    // v4 : archives tableaux commission vendeuses
    // ------------------------------------------------------------------------
    this.version(4).stores({
      vendorCommissionArchives: 'id, sessionId, archivedAt, sessionName'
    });
